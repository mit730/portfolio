...
const ProjectCard = ({ id, title, description, techStack, liveLink, image }: { id: string, title: string, description: string, techStack: string, liveLink: string, image?: StaticImageData | string }) => (
  <div className="bg-white dark:bg-gray-800 rounded-3xl overflow-hidden shadow-lg flex flex-col h-full transition-colors duration-300">
    <div className="h-48 sm:h-52 bg-gray-200 dark:bg-gray-700 relative">
      {image ? (
    ...
    <div className="p-5 sm:p-6 flex flex-col flex-grow">
      <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-3 sm:mb-4">{title}</h3>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mt-auto">
        <a target='_blank' href={liveLink} className="flex items-center gap-1.5 sm:gap-2 text-gray-800 dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400 underline text-sm font-medium transition-colors">
          <FaLink size={14} /> Live Preview
        </a>
        <Link href={`/projects/${id}`} className="flex items-center gap-1.5 sm:gap-2 text-gray-800 dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400 underline text-sm font-medium transition-colors">
          View Detail
        </Link>
      </div>
    </div>
    ...
    <section className="w-full py-12 sm:py-16 md:py-20" id="projects">
      <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
        <div className="text-center mb-10 sm:mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold text-[#42446E] dark:text-[#A6A8D6] mb-3 sm:mb-4">Projects</h2>
          <p className="text-lg sm:text-xl text-gray-500 dark:text-gray-400">Things I&apos;ve built so far</p>
        </div>
